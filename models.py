...
class Exercise(ForgeBaseModel):
    ...
    # targets are forced even (both ends rounded UP); each set logs one "reps per
    # side" value. Timed per-side holds keep their seconds untouched. See
    # services.progression.rep_targets_for.
...
class AssessmentResult(ForgeBaseModel):
    ...
    # Every anchor is a bilateral movement, so one value per pattern.
